...
def _build_command_matrix(rc_version: str, fast: bool) -> List[CommandSpec]:
    if fast:
        return [
            ...
            CommandSpec(
                "milestone",
                "rc-artifacts",
                f"python3 scripts/build_release_artifacts.py --version {shlex.quote(rc_version)} --output-dir tmp/releases_fast",
            ),
        ]
    ...
    return [
        ...
        CommandSpec("milestone", "rc-artifacts", f"make release-artifacts RELEASE_VERSION={shlex.quote(rc_version)}"),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run M25 v1.0 release candidate gate checks")
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--rc-version", default="1.0.0-rc.1")
...
def main() -> int:
    ...
    command_results = []
    for spec in _build_command_matrix(rc_version=args.rc_version, fast=args.fast):
