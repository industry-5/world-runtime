...
class OperatorWorkflowRunner:
    ...
    def _load_default_events(self) -> None:
        for adapter in self.registry.list():
            events_path = adapter.scenario_dir(self.repo_root) / "events.json"
            if not events_path.exists():
                continue

            stream_prefix = adapter.adapter_id.replace("adapter-", "")
            events = load_json(events_path)
            for index, event in enumerate(events):
                self.store.append(f"{stream_prefix}.stream.{index}", event)
