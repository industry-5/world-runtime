...
Backfill and verify the existing air-traffic adapter implementation as a package-local `M1` slice with truthful docs, aligned validation, and clear boundaries.

## In Scope

- package-local normalization of the existing adapter contract and docs
- schemas, policy, and minimal scenario alignment
- registry/example/test expectations for the current air-traffic slice
...
- `python3 -m pytest -q tests/test_adapters.py tests/test_scenario_bundles.py tests/test_air_traffic_domain.py`
